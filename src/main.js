...
async function getLatestVersion() {
    ...
    if(response.status !== 200) {
        ...
        try {
            ...
            let latestTagSHA = execSync(`git -C ${repoPath} rev-list --tags --max-count=1`).toString().trim();
...
async function getIspc(version, platform, architecture) {
    ...
    let archiveName = `ispc-${version}-${platform}${archStr}`;
    let url = `https://github.com/ispc/ispc/releases/download/${version}/${archiveName}${extension}`;
    ...
    let binDir = resolve(dir, archiveName, "bin");
    return binDir;
}
